...
export function LoginScreen({ onGoogleLogin, onEnterAsGuest }: LoginScreenProps) {
  ...
  return (
    <div className="min-h-screen w-full bg-gradient-to-br from-background via-card to-background text-foreground flex flex-col justify-between p-4 sm:p-6 lg:p-12 relative overflow-hidden">
        ...
        <div className="flex items-center gap-3">
          ...
          <div>
            <div className="flex items-center gap-2">
              <span className="font-serif text-2xl font-bold tracking-tight">50-30-20</span>
              <span className="rounded-md bg-primary/15 px-2 py-0.5 text-[10px] font-extrabold uppercase tracking-wider text-primary">
                Grupo Walnut
              </span>
            </div>
            <p className="text-[11px] font-medium text-muted-foreground">Plataforma de Finanzas Personales & Hogar</p>
          </div>
        </div>
      </header>

      {/* Main Login Hero Content */}
      <main className="flex-1 flex items-center justify-center py-10 relative z-10 max-w-5xl mx-auto w-full">
        <div className="grid lg:grid-cols-2 gap-10 lg:gap-16 items-center w-full">
          ...
          <div className="space-y-6 text-left">
            <div className="inline-flex items-center gap-2 rounded-full border border-primary/20 bg-primary/10 px-3.5 py-1.5 text-xs font-bold text-primary">
              <Sparkles size={14} /> Regla Financiera 50 / 30 / 20
            </div>

            <h1 className="font-serif text-3xl sm:text-4xl lg:text-5xl font-extrabold tracking-tight text-foreground leading-[1.15]">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
              <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card/60 p-3.5 backdrop-blur-xs">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-blue-500/15 text-blue-500">
                  <PieChart size={18} />
                </span>
                <div>
                  <p className="text-xs font-bold text-foreground">Regla 50/30/20</p>
                  <p className="text-[11px] text-muted-foreground">Presupuestos visuales</p>
                </div>
              </div>

              <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card/60 p-3.5 backdrop-blur-xs">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-purple-500/15 text-purple-500">
                  <Users size={18} />
                </span>
                <div>
                  <p className="text-xs font-bold text-foreground">Hogar Compartido</p>
                  <p className="text-[11px] text-muted-foreground">Para 2 a 3 personas</p>
                </div>
              </div>
              ...
              <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card/60 p-3.5 backdrop-blur-xs">
                ...
                <div>
                  ...
                  <p className="text-[11px] text-muted-foreground">Metas con calculadora</p>
                </div>
              </div>

              <div className="flex items-center gap-3 rounded-2xl border border-border/80 bg-card/60 p-3.5 backdrop-blur-xs">
                <span className="grid h-9 w-9 shrink-0 place-items-center rounded-xl bg-amber-500/15 text-amber-500">
                  <ShieldCheck size={18} />
                </span>
                <div>
                  <p className="text-xs font-bold text-foreground">Nube PostgreSQL</p>
                  <p className="text-[11px] text-muted-foreground">Seguridad en tiempo real</p>
                </div>
              </div>
            </div>
          </div>
          ...
          <div className="w-full max-w-md mx-auto">
            <div className="rounded-3xl border border-border bg-card/90 p-7 sm:p-9 shadow-2xl backdrop-blur-md space-y-6 text-center">
              
              <div className="space-y-2">
                <span className="grid h-14 w-14 mx-auto place-items-center rounded-2xl bg-primary/15 text-primary shadow-xs">
                  <ShieldCheck size={28} />
                </span>
                <h2 className="font-serif text-2xl font-bold text-foreground">Iniciar Sesión</h2>
                <p className="text-xs text-muted-foreground">
                  Ingresa con tu cuenta de Google para acceder a tus finanzas sincronizadas.
                </p>
              </div>
              ...
              <button
                ...
                className="flex h-11 w-full items-center justify-center gap-2 rounded-2xl bg-secondary/80 text-xs font-bold text-foreground hover:bg-secondary transition focus-ring"
              >
                Explorar en Modo Demo <ArrowRight size={15} />
              </button>

              <p className="text-[10px] text-muted-foreground pt-1">
                Al continuar aceptas los términos y condiciones de <strong>Grupo Walnut</strong>.
              </p>
            </div>
          </div>
      ...
      {/* Footer */}
      <footer className="text-center text-xs text-muted-foreground relative z-10 max-w-6xl mx-auto w-full pt-6 border-t border-border/40">
        <p>50-30-20 — Una plataforma desarrollada por <strong className="text-foreground font-semibold">Grupo Walnut</strong> · 2026</p>
      </footer>
    </div>
  );
}
