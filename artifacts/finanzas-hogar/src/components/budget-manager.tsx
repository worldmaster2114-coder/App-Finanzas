...
import { Plus, AlertTriangle, CheckCircle2, XCircle, Calendar, DollarSign, X, Pencil, Trash2 } from 'lucide-react';
...
export function BudgetManager({
  ...
  const handleOpenEdit = (budget: Budget) => {
    ...
    setEditCategoryId(budget.categoryId || categories.filter((c) => c.type === 'expense')[0]?.id || '');
    setEditLimitAmount(budget.amountLimit.toString());
    setEditAlertThreshold((budget.alertThreshold || 80).toString());
  };
  ...
  const handleSaveEdit = (e: React.FormEvent) => {
    ...
    onUpdateBudget({
      ...
      amountLimit: limit,
      alertThreshold: parseInt(editAlertThreshold) || 80,
    });
  ...
  const handleDelete = (budget: Budget) => {
    ...
    const catName = cat?.name || 'este presupuesto';
    if (window.confirm(`¿Estás seguro de que deseas eliminar el presupuesto de "${catName}"?`)) {
    ...
    <div className="space-y-6">
      ...
      {budgets.length === 0 ? (
        <div className="flex flex-col items-center justify-center rounded-3xl border border-dashed border-border py-16 text-center">
          <p className="text-sm font-semibold text-foreground">No tienes presupuestos creados aún</p>
          <p className="mt-1 text-xs text-muted-foreground">Crea presupuestos mensuales para tus categorías de gasto para controlar tus finanzas.</p>
          <button
            onClick={() => setIsAddModalOpen(true)}
            className="mt-4 flex items-center gap-2 rounded-xl bg-primary px-4 py-2 text-xs font-bold text-primary-foreground shadow-xs hover:brightness-105"
          >
                  ...
                  <div className="flex items-center justify-between">
                    ...
                    <div className="flex items-center gap-1.5">
                      ...
                      <button
                        ...
                        title="Editar límite o categoría"
                      >
                        <Pencil size={14} />
                      </button>
                      <button
      ...
      {editingBudget && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/50 p-4 backdrop-blur-xs">
          <form onSubmit={handleSaveEdit} className="w-full max-w-sm rounded-3xl border border-border bg-card p-6 shadow-2xl animate-in zoom-in-95">
            <div className="flex items-center justify-between pb-3 border-b border-border">
              <h3 className="font-serif text-lg font-bold">Editar Presupuesto</h3>
              <button type="button" onClick={() => setEditingBudget(null)} className="grid h-8 w-8 place-items-center rounded-full bg-secondary text-muted-foreground hover:text-foreground">
                <X size={16} />
              </button>
            </div>
