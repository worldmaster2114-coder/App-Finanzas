...
export function OnboardingWizard({ isOpen, onClose, initialUser, onOpenGoogleLogin, onComplete }: OnboardingWizardProps) {
      ...
      <div className="w-full max-w-xl max-h-[90dvh] overflow-y-auto rounded-3xl border border-border bg-card p-6 shadow-2xl animate-in zoom-in-95 sm:p-8">
        ...
        {step === 1 && (
          <div className="mt-6 space-y-5 animate-in fade-in-50">
            {initialUser?.email && (
              <div className="rounded-2xl border border-primary/20 bg-primary/5 p-3 flex items-center gap-2.5 text-xs text-primary font-bold">
                <Check size={16} /> Sesión iniciada como: <span className="text-foreground">{initialUser.name || initialUser.email}</span>
              </div>
            )}
