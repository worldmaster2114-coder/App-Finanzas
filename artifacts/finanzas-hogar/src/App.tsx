...
function AppShell({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="min-h-[100dvh] bg-background md:flex">
       <aside className="app-sidebar hidden md:flex md:w-[240px] lg:w-[268px] shrink-0 flex-col bg-sidebar px-5 py-6 text-sidebar-foreground">
       ...
       <nav className="compact-desktop-nav" aria-label="Navegación principal">
         <div className="compact-desktop-nav__inner">
           <button onClick={() => goTo('dashboard')} className="flex items-center gap-2.5 text-left focus-ring" data-testid="button-compact-brand">
             <span className="grid h-9 w-9 place-items-center rounded-[11px] bg-sidebar-primary text-sidebar-primary-foreground"><Wallet size={18} strokeWidth={2.3} /></span>
             <span className="font-serif text-[21px] leading-none">Al día</span>
           </button>
           <div className="flex items-center gap-1.5">
             <button onClick={() => goTo('dashboard')} className="flex items-center gap-2 rounded-xl bg-sidebar-accent px-4 py-2.5 text-xs font-bold text-sidebar-accent-foreground transition hover:bg-sidebar-accent/80 focus-ring" data-testid="button-compact-nav-dashboard"><LayoutDashboard size={16} />Resumen<span className="h-1.5 w-1.5 rounded-full bg-sidebar-primary" /></button>
             <button onClick={() => goTo('history')} className="flex items-center gap-2 rounded-xl px-4 py-2.5 text-xs font-semibold text-sidebar-foreground/70 transition hover:bg-sidebar-accent/60 hover:text-sidebar-foreground focus-ring" data-testid="button-compact-nav-history"><BarChart3 size={16} />Historial</button>
           </div>
           <button onClick={() => window.dispatchEvent(new CustomEvent('open-transaction'))} className="flex items-center gap-2 rounded-xl bg-sidebar-primary px-4 py-2.5 text-xs font-bold text-sidebar-primary-foreground transition hover:brightness-95 focus-ring" data-testid="button-compact-add"><Plus size={16} />Añadir movimiento</button>
         </div>
       </nav>
